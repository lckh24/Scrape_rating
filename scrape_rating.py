...
# cred_path = r"C:\Users\lecon\AppData\Roaming\gspread_pandas\google_secret.json" #Input the credential path

# # chromium_path = os.getenv("CHROMIUM_PATH")
# #executable_file = 'chrome.exe'
# executable_path = r"C:\Users\lecon\AppData\Local\ms-playwright\chromium-1129\chrome-win"
# headless_option = False
# chosen_cookies = "laz_cookies.txt"
# sort_option = 'recent'

# #Value need to prepared
# spreadsheet_name = "scrape_rating_laz"
# worksheet_name = "Sheet1"

# web_socket_path='ws://localhost:9222/devtools/browser/fc145694-511b-4389-84e3-19fa6a9a88e9'
...
cred_path = r"C:\Users\lecon\AppData\Roaming\gspread_pandas\google_secret.json"
...
start_rows = 2
end_rows = 153
get_current_rating = True


def get_text_map():
    return {
        'weeks': 7, 'minutes': 1/1440, 'minute': 1/1440, 'week': 7,
        'hour': 1/24, 'hours': 1/24, 'days': 1, 'day': 1
    }
...
def get_total_ratings(page):
    try:
        page.wait_for_selector('div.pdp-review-summary a', timeout=3000)
        rating_text = page.query_selector('div.pdp-review-summary a').text_content()
        if "k" in rating_text:
            return float(re.search(r'(\d+\.?\d*)', rating_text).group(1)) * 1000
        return int(re.search(r'(\d+)', rating_text).group(1))
    except Exception:
        return 0
...
def get_selling_price(page):
    try:
        page.wait_for_selector('div.pdp-product-price span', timeout=3000)
        price_text = page.query_selector('div.pdp-product-price span').text_content().strip()
        cleaned_price = re.sub(r'[^\d.,]', '', price_text)

        if ',' in cleaned_price and '.' in cleaned_price:
            last_comma = cleaned_price.rfind(',')
            last_dot = cleaned_price.rfind('.')
            normalized_price = (cleaned_price.replace('.', '') if last_comma > last_dot
                                else cleaned_price.replace(',', ''))
        elif ',' in cleaned_price:
            normalized_price = cleaned_price.replace(',', '.') if len(cleaned_price.split(',')[-1]) < 3 else cleaned_price.replace(',', '')
        elif '.' in cleaned_price:
            normalized_price = cleaned_price.replace('.', '.') if len(cleaned_price.split('.')[-1]) < 3 else cleaned_price.replace('.', '')
...
def to_sort(page, sort_option):
    try:
        page.wait_for_selector('div.oper', timeout=3000)
        page.query_selector_all('div.oper')[1].click()
        page.wait_for_selector('ul.next-menu-content', timeout=3000)
        for li in page.query_selector('ul.next-menu-content').query_selector_all('li'):
            if li.text_content().strip().lower() == sort_option.lower():
                li.click()
                time.sleep(1)
                return "Sorted"
    except Exception as e:
...
with sync_playwright() as p:
    browser = p.chromium.launch(headless=headless_option, executable_path=executable_path)
